...
describe List do
  ...
  it 'creates an array of rows off of every item' do
    ...
    @list.rows()[:rows].count.should.equal? 5
  end
  ...
  it 'will return a limited number if limit is set' do
    ...
    @list.rows(:limit => 1)[:rows].count.equal? 1
  end
  ...
  it 'will return an offset number if offset is set' do
    ...
    @list.rows(:offset => 1)[:rows][0].key.should.equal? @list.items[2].id
  end
  ...
  it 'using the rows method with sort will return rows sorted by own field in correct order' do
    ...
    x = 0
    @child.rows(:sort => @sortable.name)[:rows].each do |r|
  ...
  describe 'using the rows method with sort on a grandparents field' do
    ...
    it 'will return rows sorted by grandparents with nil at the end when sorted acs' do
      ...
      @grandchild.rows(:sort => {:field => @sortable.name, :direction => 'ASC'} )[:rows].first.key.should == @row_save[:key]
    end
    it 'will return rows sorted by grandparents with nil at the end when sorted desc' do
      x = 0
      @row_save = Row.new(:list => @grandchild).save
      @grandchild.rows(:sort => {:field => @sortable.name, :direction => 'DESC'} )[:rows].last.key.should == @row_save[:key]
    end
    it 'will return a limited number if limit is set' do
      @grandchild.rows(:sort => {:field => @sortable.name, :direction => 'DESC'} , :limit => 1)[:rows].count.equal? 1
    end
    it 'will return the correct row if a limit is set' do
      @grandchild.rows(:sort => {:field => @sortable.name, :direction => 'DESC'} , :limit => 1)[:rows].first[@sortable.name].equal? @vals.last
    end
    it 'will offset when a sort is performed' do
      @grandchild.rows(:sort => {:field => @sortable.name, :direction => 'DESC'}, :offset => 1)[:rows].first[@sortable.name].equal? @vals[@vals.length - 1]
    end
    it 'will return correct rows when multi sort is performed' do
      ...
      @grandchild.rows(:sort => [Field.first.id, Field.last.to_param ] )[:rows].first.key.should == @row[:key]
    end
  end
  ...
  describe 'the row :filter functionality' do
    ...
    it 'filters rows based on equal criteria' do 
      ...
      @list.rows(:filter => Field.first.to_param+' is preserved' )[:rows].first.key.should == @row[:key]
    end
    it 'filters rows based on nested equal criteria' do 
      ...
      @row = Row.new(:list => @list, Field.first.name => 'preserved').save
      @list.rows(:filter => [Field.first.to_param+' is preserved', Field.last.to_param+' is not filtered'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on not empty' do
      Row.new(:list => @list).save
      @row = Row.new(:list => @list, Field.first.name => 'preserved').save
      @list.rows(:filter => [Field.first.to_param+' is not empty'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on empty' do
      Row.new(:list => @list, Field.first.name => 'stuff here').save
      @row = Row.new(:list => @list).save
      @list.rows(:filter => [Field.first.to_param+' is empty'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on parent field' do
      ...
      @child.rows(:filter => [Field.first.to_param+' is want'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on starts with' do
      ...
      @list.rows(:filter => [Field.first.to_param+' starts with stuff'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on ends with field' do
      Row.new(:list => @list, Field.first.name => 'stuff bear').save
      @row = Row.new(:list => @list, Field.first.name => 'stuff here').save
      @list.rows(:filter => [Field.first.to_param+' ends with here'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on greater than' do
      ...
      @list.rows(:filter => [Field.first.to_param+' greater than a'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on less than' do
      Row.new(:list => @list, Field.first.name => 'd').save
      @row = Row.new(:list => @list, Field.first.name => 'a').save
      @list.rows(:filter => [Field.first.to_param+' less than d'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on greater than or equal to' do
      Row.new(:list => @list, Field.first.name => 'a').save
      @row = Row.new(:list => @list, Field.first.name => 'c').save
      @list.rows(:filter => [Field.first.to_param+' greater than or equal to c'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters based on less than or equal to' do
      Row.new(:list => @list, Field.first.name => 'd').save
      @row = Row.new(:list => @list, Field.first.name => 'b').save
      @list.rows(:filter => [Field.first.to_param+' less than or equal to b'] )[:rows].first.key.should == @row[:key]
    end
    it 'filters with a limit' do
      Row.new(:list => @list).save
      @row = Row.new(:list => @list, Field.first.name => 'a').save
      Row.new(:list => @list, Field.first.name => 'a').save
      @list.rows(:filter => [Field.first.to_param+' == a'], :limit => 1)[:rows].last.key.should == @row[:key]
    end
    it 'filters with a limit and an offset' do
      Row.new(:list => @list).save
      Row.new(:list => @list, Field.first.name => 'a').save
      @row = Row.new(:list => @list, Field.first.name => 'a').save
      @list.rows(:filter => [Field.first.to_param+' is a'], :limit => 1, :page => 1 )[:rows].first.key.should == @row[:key]
    end
    it 'filters work fine with spaces as longs as they are in quotes' do
      ...
      @list.rows(:filter => [Field.first.to_param+' is "farting is good"'])[:rows].first.key.should == @row[:key]
    end
  end
