...
describe("the bindings on fields", function() {
  ...
  describe("the text bindings", function() {
      it("opens and is changed", function() {
 		spyOnEvent($('.data.text:visible'), 'open');
  		$('.text:visible').click()
 		expect('open').toHaveBeenTriggeredOn($('.text:visible'));
      });
    });
  describe("the select bindings", function() {
   ...
   describe("the number bindings", function() {
     ...
     it("updates when up is clicked", function() {
 		newRows()[0][number](2);
		$('.scroller .number_up').click();
 		expect( $('.number textarea') ).toHaveValue(3);
     });
     it("updates when changed above", function() {
 		newRows()[0][number](2);
		$('.scroller .number_down').click();
   ...
   describe("the suggest bindings", function() {
     it("opens and is changed", function() {
 		...
 		$('.scroller textarea.suggest').val("SuggestMe").change().focus();
 		expect( $('.scroller textarea.suggest').attr('aria-haspopup') ).toEqual('true');
  ...
  describe("the date bindings", function() {
    it("renders a blank row as -- not a date", function() {
		expect( $('.scroller textarea.date').click().val() ).toEqual('--')
    });
    it("opens the calendar when date textarea", function() {
		$('.scroller textarea.date').click();
		expect( $('.scroller textarea.date:first').next('div') ).toContain('div.ui-datepicker-inline');
    });
    ...
    it("presses down button date back", function() {
		...
		press.ctrlKey = false, 
			press.keyCode = 40;
		$('.scroller textarea.date:first').trigger(press);
