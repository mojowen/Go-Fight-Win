 ...
 describe 'POST update to a list' do
   ...
   it 'updates grandchild row and updates parent and grandparent items' do
     ...
     @rows = '{
      ...
      "1": {"key":"'+@row2[:key].to_s+'","list":"'+@list.name+'","'+@list.fields[2].to_param+'":"new different value", "'+@grandchild.fields[3].to_param+'":"grandchild different value"}
     }'
     post 'update',  :org_name => @org.to_param, :list_name => @list.to_param, :rows => @rows, :format => :json
   ...
   it 'returns success for result for all success' do
     ...
     @rows = '{
      ...
      "2": {"key":"'+@row1[:key].to_s+'","list":"'+@grandchild.name+'","'+@list.fields[0].to_param+'":"old value"},
      "4": {"key":"new","list":"'+@grandchild.name+'"},
      "3": {"key":"'+@row2[:key].to_s+'","list":"'+@grandchild.name+'","'+@list.fields[2].to_param+'":"old different value"}
     }'
